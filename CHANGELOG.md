...
Nothing yet. The repository builds as `1.0.1-dev.N`, where `N` is the number of commits since the
last tag.

## [1.0.0] - 2026-08-22

First release. The API is stable: it took its final shape while the library was being built against a
real application - a point-cloud editor whose whole ribbon runs on it - and the private
`0.1.0-dev.*` packages were where it changed. Three of the entries below say what a dev build got
wrong and what it cost to find out; they are here because that is where the shape came from.
...
#### Tabs, and how much of the ribbon is drawn
...
#### What the ribbon says on its own account
