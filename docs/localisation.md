...
the ribbon says on its own account is seven sentences, and they are properties on `RibbonStrings`.

Nothing a user reads comes from a resource key, so there is no dictionary to merge for this. Set the
seven from code, once, from wherever the application keeps its translations.
...
name and `ContextualTabNameFormat` takes the tab's; all three must keep their `{0}`. A translation
that drops it produces a launcher every screen reader calls the same thing, which is the failure the
sentence exists to avoid. `LocalisationTests` checks that every translation below still has it.

Three of the seven are never seen and only heard: `GroupLauncherNameFormat`,
`CollapsedGroupNameFormat` and `ContextualTabNameFormat` are automation names. They are the ones
worth getting right for somebody who cannot see the icon that is beside them — or, in the last case,
cannot see that the strip has a name on it that was not there a moment ago, which is the whole point
of a contextual tab. Each language below uses the words Office uses in it for one: *pestaña
contextual*, *onglet contextuel*, *kontextbezogene Registerkarte*.
...
The seven are read by `LocalisationTests`, which also checks that this file mentions every property of
